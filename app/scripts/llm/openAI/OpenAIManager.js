      ...
      if (_.isNull(Swal)) {
        ...
        Swal.fire({
          ...
          onOpen: async () => {
            ...
            chrome.runtime.sendMessage({ scope: 'llmQuestion', cmd: 'askLLMOpenAI', data: {documents: documents, apiKey: apiKey, query: query} }, ({ res }) => {
              Swal.close()
      ...
      if (_.isNull(Swal)) {
        ...
        Swal.fire({
          ...
          onOpen: async () => {
            ...
            chrome.runtime.sendMessage({ scope: 'llmQuestion', cmd: 'askLLMOpenAI4', data: {documents: documents, apiKey: apiKey, query: query} }, ({ res }) => {
              Swal.close()
              const jsonString = res.text
              console.log('ANSWER: ' + jsonString)
              let retrievedJSON = jsonString.substring(jsonString.indexOf('{') + 1)
